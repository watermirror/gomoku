...
- Q: 黑方落子时，棋盘是否应主动标记禁手位置？ → A: 不主动标记，仅在尝试落子时被动拒绝
- Q: 禁手落子被拒绝时的反馈方式？ → A: 双重反馈——该位置短暂闪烁红色提示（约 300ms），同时右侧栏显示"禁手位置，无法落子"文字提示
- Q: 和局后玩家可执行什么操作？ → A: 悔棋+返回，与胜负后行为一致
- Q: 进场和退场滑动动画的持续时长？ → A: 500ms
- Q: 右侧栏显示哪些内容？ → A: 仅核心信息——当前回合方、落子计数、操作按钮（悔棋/返回）
...
4. **Given** 玩家已选择全部启用禁手并开始对局, **When** 黑方尝试在禁手位置落子, **Then** 落子被拒绝，该位置短暂闪烁红色提示，右侧栏显示"禁手位置，无法落子"文字提示，回合不切换
...
- 棋盘下满所有 225 个位置且无五子连线时，应判定为和局，右侧栏显示和局信息，棋盘禁止继续落子，但玩家可悔棋或返回（与胜负后行为一致）
- 落子到已有棋子的位置应被忽略，不产生任何效果
- 黑方只有一手棋且当前为白方回合时，悔棋应撤回黑方的唯一一手棋
- 棋盘不主动显示禁手位置标记；禁手仅在黑方尝试落子时进行校验并拒绝
...
- **FR-006**: 进场动画必须将标题面板和模式按钮滑出屏幕，将棋盘从居中滑动到窗口左侧，将右侧栏从窗口右侧外滑入，动画时长 500ms
- **FR-007**: 退场动画必须将右侧栏滑出屏幕，将棋盘从左侧滑回居中，将标题面板和按钮从左侧滑入恢复标题画面，动画时长 500ms
- **FR-008**: 所有动画播放期间必须使用遮盖层屏蔽用户交互
- **FR-009**: 对局中黑先白后交替落子，玩家点击棋盘空位落子
- **FR-010**: 落子后系统必须判定是否形成五子连线，若是则宣布该方胜利
- **FR-011**: 对局中右侧栏必须显示：当前回合方、落子计数、状态信息（用于胜负/和局/禁手提示）、悔棋按钮、返回按钮
- **FR-012**: 胜负后必须在右侧栏显示获胜方信息
- **FR-013**: 对局中有棋子时必须提供可用的"悔棋"按钮，无棋子时禁用
- **FR-014**: 悔棋必须撤回最后一手棋，包括在胜负已分或和局后也能悔棋使棋局恢复进行中状态
- **FR-015**: 必须提供"返回"按钮，点击后触发退场动画回到标题画面
- **FR-016**: 返回标题画面后，对局数据必须被完全清除
- **FR-017**: 落子合法性必须遵循选定的禁手规则配置（仅对黑方生效的三三、四四、长连禁手）
- **FR-018**: 黑方尝试在禁手位置落子被拒绝时，必须在该位置短暂闪烁红色提示，并在右侧栏显示"禁手位置，无法落子"文字提示
